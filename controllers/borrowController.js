...
const formatBorrowRecord = (record) => {
    if (!record) return null;
    const effectiveStatus = calculateStatus(record);
    return {
        ...record,
...
export const getAllBorrowRecords = asyncHandler(async(req,res)=>{
    ...
    let sql = `
        ...
        books.id AS book_id, books.title AS book_title, books.isbn AS book_isbn
        FROM borrow_records
        LEFT JOIN readers ON borrow_records.reader_id = readers.id
    ...
    if(status === 'overdue'){
        searchFields.push(`(borrow_records.status = 'overdue' OR (borrow_records.status = 'borrowed' AND borrow_records.due_date < ?))`);
        params.push(currentDate);
    } else if(status === 'borrowed'){
        searchFields.push(`borrow_records.status = 'borrowed' AND borrow_records.due_date >= ?`);
        params.push(currentDate);
    ...
    if (sort && sortBy.includes(sort)) {
        sql += ` ORDER BY borrow_records.${sort} ${order}`;
    } else {
        sql += ` ORDER BY borrow_records.created_at ${order}`;
    }
    sql += ` LIMIT ? OFFSET ?`
    ...
    const formattedRecords = records.map(formatBorrowRecord);
...
export const getSingleBorrowRecord = asyncHandler(async(req,res)=>{
    ...
    const formattedRecord = formatBorrowRecord(record);
    const result = {
        id: formattedRecord.id,
        borrow_date: formattedRecord.borrow_date,
        due_date: formattedRecord.due_date,
        return_date: formattedRecord.return_date,
        status: formattedRecord.status,
        is_overdue: formattedRecord.is_overdue,
        created_at: formattedRecord.created_at,
        updated_at: formattedRecord.updated_at,
        reader: {
            id: formattedRecord.reader_id,
            name: formattedRecord.reader_name,
            email: formattedRecord.reader_email,
            phone: formattedRecord.reader_phone
        },
        book: {
            id: formattedRecord.book_id,
            title: formattedRecord.book_title,
            isbn: formattedRecord.book_isbn,
            published_year: formattedRecord.published_year
        }
    };
    
    logger.info(`Borrow record retrieved successfully`);
    return res.status(200).json({msg:'Borrow record retreived sucessfully', data : result});
});

export const updateOverdueStatuses = asyncHandler(async(req, res) => {
    logger.info('Starting overdue status update batch job');
    ...
    await execute(db, updateSQL, [currentDate]);
    
    logger.info('Overdue status update completed');
    return res.status(200).json({msg:'Overdue statuses updated successfully'});
});
